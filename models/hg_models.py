...
class Attn3_hard_coded(nn.Module):
    seq_len: int
    qkv_dim: int
    drop_out: float = 0.1
    
    def setup(self):
        self.q_linear = jnp.ones((self.seq_len, self.seq_len))
        self.q_linear = 0.5 * self.q_linear
        self.q_linear.at[-1, :].set(1)
        self.v_linear = nn.Dense(self.seq_len)
...
class AttnModel3_hc(nn.Module):
    ...
    def setup(self):
        ...
        self.attn = Attn3_hard_coded(self.N * 2 + 2, self.qkv_features, self.drop_out)
